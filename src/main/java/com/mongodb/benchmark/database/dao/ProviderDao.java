...
public class ProviderDao {
    private static final Logger logger = LoggerFactory.getLogger(ProviderDao.class.getName());
    private static MongoCollection<Document> localCollection = null;
    private static final String coll = "providers";
    ...
    public static Document find(int id) {
        ...
        return localCollection.find(com.mongodb.client.model.Filters.eq("data.Provider.ID", "P-" + id)).first();
    }
    ...
    public static Document findOneAndReplace(ClientSession sess, int id, String oldVersion, Document doc) {
        ...
        return localCollection.findOneAndReplace(sess,
                and(eq("data.Provider.I", "P-" + id), eq("version", oldVersion)),
                doc);
    }
